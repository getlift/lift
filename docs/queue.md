...
constructs:
    my-queue:
        # ...
        name: my-awesome-queue # Will set the queue name with this value and 'my-awesome-queue-dlq' for DLQ queue
```

In some use cases it is useful to have the ability to set a custom name for our queues, this can be done by providing the `name` property to the queue construct. This property is also compatible with FIFO queues.

Although, queue names must contain alphanumeric characters, hyphens or underscores and no more than 80 characters including suffixes, that means a maximum of 76 characters for non FIFO queues and 71 characters for FIFO queues. Lift will throw an error if you violate any of those restrictions.

You can learn more about the name limits in the [official AWS documentation](https://docs.aws.amazon.com/AWSSimpleQueueService/latest/APIReference/API_CreateQueue.html).
