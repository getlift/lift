...
import { CommandsDefinition, Hook, VariableResolver } from "./types/serverless";
...
class LiftPlugin {
    ...
    constructor(serverless: Serverless) {
        ...
        this.hooks = {
            ...
            "constructs:deploy:deploy": this.deploy.bind(this),
            "before:deploy:deploy": this.deploy.bind(this),
            "after:remove:remove": this.remove.bind(this),
            "constructs:remove:remove": this.remove.bind(this),
        };
    ...
    async deploy(): Promise<void> {
        ...
        for (const [id, component] of Object.entries(this.components)) {
            console.log(chalk.yellow(`Deploying ${id}`));
    ...
    async remove(): Promise<void> {
        // TODO in correct order with object graph
        for (const [id, component] of Object.entries(this.components)) {
            console.log(chalk.yellow(`Removing ${id}`));
